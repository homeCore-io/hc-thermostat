//! Configurable log rotation and compression for hc-thermostat.
//!
...
pub struct LoggingConfig {
    ...
    /// Accepts any `tracing` filter directive, e.g. `"debug"` or `"hc_thermostat=debug,rumqttc=warn"`.
...
/// - `prefix`: log file name prefix, e.g. `"hc-thermostat"` -> `logs/hc-thermostat.log`.
/// - `stderr_default`: filter used for stderr when `RUST_LOG` is unset and
///   `cfg.level` is the default `"info"`, e.g. `"hc_thermostat=info"`.
